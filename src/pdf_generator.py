#!/usr/bin/env python3
"""Module sinh PDF hướng dẫn lưu trữ từ file JSON — gói gọn trong 1 trang A4.

Có thể dùng như module từ code khác hoặc CLI:
    python -m src.pdf_generator template_pdf.json [output.pdf]
"""
...
def generate_pdf_from_template(
    ...
    """Đọc file JSON template và sinh file PDF — gói gọn trong 1 trang A4.

    Đây là API tương thích với template_engine.
    Tham số project_name sẽ hiển thị thay document_name nếu có.

    Args:
        json_path: Đường dẫn file template_pdf.json.
        output_path: Đường dẫn file PDF đầu ra.
        project_name: Tên dự án (hiển thị trên tiêu đề).
...
if __name__ == "__main__":
    ...
    result = generate_pdf_from_template(
        json_path,
        output_path or Path("output.pdf"),
        project_name="",
    )
    ...
    size_str = f"{result.stat().st_size:,} bytes"
    print(f"PDF da tao: {result}")
    print(f"   Kich thuoc: {size_str}")
    print(f"   So trang: {page_count}")
    if page_count == 1:
        print("PDF nam gon trong 1 trang A4!")
    else:
        print(f"PDF co {page_count} trang. Can tinh chinh them.")
